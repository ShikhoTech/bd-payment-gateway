...
func TestCreateSessionV4(t *testing.T) {

	storeId := os.Getenv("SSLCOM_STORE_ID")
	storePass := os.Getenv("SSLCOM_STORE_PASSWORD")

	sslCom := sslcom.GetSslCommerzV4(storeId, storePass, false)
	...
	sessionResponse, err := sslCom.CreateSession(sessionRequest)
	if err != nil {
		t.Error(err.Error())
	}
	...
	if sessionResponse.Status != "SUCCESS" {
		t.Errorf("Can Not Create a Session with V4")
	}
}
